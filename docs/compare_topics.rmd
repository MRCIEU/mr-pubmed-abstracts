...
read_data <- function(ind, search) {
    a <- read.csv(file.path("~", "Downloads", paste0("PubMed_Timeline_results_by_Year-", ind, ".csv")), skip=1, header=TRUE) %>% mutate(search = search)
    return(a)
}
...
pby <- read.csv("~/Downloads/PubMed by Year.csv", header=TRUE) %>% as_tibble()
names(pby)[-1] <- unique(dat$search)
pby <- pby %>% gather(key="search", value="count_ratio", -Year) 

dat <- left_join(dat, pby, by=c("Year", "search"))
dat <- group_by(dat, search) %>% mutate(count_ratio_norm = count_ratio/max(count_ratio)) %>% ungroup()
