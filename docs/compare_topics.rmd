...
dat <- tibble(ind=1:5, search=c("MR", "GWAS", "Systematic reviews", "Meta-analysis", "Mediation analysis")) %>% pmap(., read_data) %>% bind_rows() %>% as_tibble()
...
dat %>%
    filter(Year >= 2000 & Year <= 2024) %>%
ggplot(., aes(x=Year, y=Count, color=search)) + geom_line() + geom_point() + scale_colour_brewer(type="qual")
```
...
pby <- read.csv(here("data/pubmed_counts/PubMed by Year(1).csv"), header=TRUE) %>% as_tibble()
...
dat$count_ratio <- dat$count_ratio / max(dat$count_ratio, na.rm=T) * max(dat$Count, na.rm=T)

dat %>% 
    filter(Year >= 1985 & Year <=2024) %>%
    ggplot(., aes(x=Year, y=count_ratio, color=search)) + 
    geom_line() + 
    geom_point() + 
    scale_colour_brewer(type="qual", palette=3) +
    labs(colour="Study type", y="Annual number of studies in PubMed", x="Year") +
    theme_bw() +
    theme(legend.position=c(0.2, 0.7))
ggsave(file=here("docs/topic_growth.pdf"), width=6, height=4)
```
...
```{r}
dat2 <- group_by(dat, search) %>% mutate(count_ratio_norm = count_ratio / max(count_ratio, na.rm=T)) %>% ungroup()
dat2 %>% ggplot(., aes(x=Year, y=count_ratio_norm, color=search)) + geom_line() + geom_point() + scale_colour_brewer(type="qual")
```
...
calculate_exp_growth <- function(time, counts) {
    ...
    return(list(
        ...
        r_squared = r_squared,
        doubling_time = doubling_time %>% round(2),
...
l <- list()
l[[1]] <- dat %>% filter(Year >= 2003 & Year <= 2024) %>%
    group_by(search) %>% do({
...
l[[2]] <- dat %>% filter(Year <= 2024) %>%
    group_by(search) %>% do({
        x <- .
        calculate_exp_growth(x$Year, x$countn)
    }) %>% select(search, doubling_time_all = doubling_time)

l[[3]] <- group_by(dat, search) %>% summarise(total = sum(Count), count_2024 = max(Count), ratio = round(count_2024 / total, 2))

growth <- Reduce(function(x, y) merge(x, y, by="search"), l) %>% arrange(desc(ratio))
names(growth) <- c("Study type", "Doubling time (years) since 2003", "Doubling time (years) all time", "Total studies", "Studies in 2024", "ratio")
write.csv(growth, here("docs/growth.csv"), row.names=FALSE)
growth %>% knitr::kable()
```
