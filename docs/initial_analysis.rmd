...
pdate <- dat %>% mutate(year = year(pub_date)) %>% dplyr::select(pmid, year)
nulls <- inner_join(nulls, pdate)
group_by(nulls, year) %>% summarise(
    null = sum(null > 0, na.rm=T) ,
    nonnull = sum(nonnull > 0, na.rm=T) ,
) %>%
filter(null + nonnull != 0 & year <=2024) %>%
mutate(ratio = null / nonnull) %>%
ggplot(., aes(x=year, y=ratio)) +
geom_point() +
geom_line() +
geom_smooth(se=FALSE) +
labs(y="Ratio of papers reporting any null to any non-null fundings")
ggsave(file=here("docs/nulls.pdf"), height=6, width=6)

group_by(nulls, pmid) %>%
    summarise(n=null + nonnull, ratio=null / nonnull) %>%
    filter(n < 20) %>%
    {hist(.$n, breaks=100)}

nulls %>% filter(!duplicated(pmid)) %>% group_by(pmid, year) %>%
    summarise(n=null + nonnull, ratio=null / nonnull) %>%
    filter(n < 20 & !is.na(ratio) & ratio != 0) %>%
    ggplot(., aes(x=as.factor(year), y=ratio)) +
    geom_boxplot(outliers=FALSE)

ggplot(nulls, aes(x=year, y=null / nonnull)) +
geom_point() +
geom_smooth()
...
```{r}
temp2 <- temp1 %>% group_by(pmid) %>% mutate(category = gsub("disease of the ", "", category)) %>% do({
    expand.grid(exposure = subset(., what=="exposure")$category, outcome = subset(., what == "outcome")$category)
}) %>% group_by(exposure, outcome) %>% summarise(n=n())

ggplot(temp2, aes(x=exposure, y=outcome)) +
geom_tile(aes(fill=n))
```

```{r}
library(tidyr)
temp3 <- tidyr::pivot_wider(temp2, names_from=outcome, values_from=n, values_fill=0) %>% as.data.frame
rownames(temp3) <- temp3$exposure
temp3[,-1] %>% as.matrix() %>% heatmap
...
library(cowplot)
library(egg)
p1 <- ggplot(temp2, aes(x=exposure, y=outcome)) +
geom_point(aes(size=n)) +
theme(legend.position="none", axis.text.x=element_text(angle=45, hjust=1), axis.text.y=element_text(angle=45, hjust=1), plot.margin=margin(0,0,1,1,"cm"), panel.background=element_blank()) +
labs(x="", y="")
p1

p2 <- temp2 %>% group_by(exposure) %>%
    summarise(value=sum(n), what="Exposure trait categories") %>%
    ggplot(aes(x=exposure, y=value)) +
    geom_bar(stat="identity", position="dodge") +
    theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(), plot.margin=margin(1,0,0,1,"cm"), panel.background=element_blank(), strip.background=element_blank(), strip.text=element_text(size=15)) +
    labs(x="", y="Number of studies") +
    facet_grid(. ~ what)
p2

p3 <- temp2 %>% group_by(outcome) %>%
    summarise(value=sum(n), what="Outcome trait categories") %>%
    ggplot(aes(x=outcome, y=value)) +
    geom_bar(stat="identity", position="dodge") + 
    ylim(c(0, 6000)) +
    coord_flip() +
    theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(), plot.margin=margin(0,1,0,0,"cm"), panel.background=element_blank(), strip.background=element_blank(), strip.text=element_text(size=15)) +
    labs(y="", x="") +
    facet_grid(what ~ .)
p3
...
gg_empty <- temp2 %>%
    ...
    theme(axis.text = element_blank(),
        ...
        line = element_blank(),
        panel.background = element_blank(), plot.margin=margin(1,1,0,0,"cm"))
gg_empty

pdf(here("docs/trait_category_counts.pdf"), width=9, height=9, onefile=FALSE)
ggarrange(p2, gg_empty, p1, p3, nrow=2, ncol=2, widths=c(3,1), heights=c(1,3))
dev.off()

setEPS()
postscript(here("docs/trait_category_counts.eps"))
ggarrange(p2, gg_empty, p1, p3, nrow=2, ncol=2, widths=c(3,1), heights=c(1,3))
dev.off()
